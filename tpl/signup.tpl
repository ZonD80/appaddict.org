{include file='doctype.tpl'}<head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>AppAddict - {$API->LANG->_('Signup')}</title>
    <style type="text/css">
  ...
  <body>
   ...
   <div class="loadingbox">
     <div class="roundtop"><img width="13" height="13" alt="" class="corner" style="display: none" src="./message_files/tl.jpg"></div>
      <p class="title">{$API->LANG->_('Signup')}</p>
      <p class="subtitle">{$API->LANG->_('Please provide this information')} ({$API->LANG->_('all fields are required')}):</p>

      <center>
          
          {$API->LANG->_('Nickname')}:<input type="text" name="name" required="required" placeholder="{$API->LANG->_('Nickname')}"/><br/>
          {$API->LANG->_('E-mail')}:<input type="email" name="email" required="required" placeholder="{$API->LANG->_('E-mail')}"/><br/>
          <span style="color:red;">Please do not use HOTMAIL, LIVE, OUTLOOK.COM email, Microsoft bounces our emails due to their stupidity. BTW, we disrespect Windows.</span><br/>
        {$API->LANG->_('Password will be sent to this email address')}<br/>
        {$captcha}</center>
       ...
       <div class="clear"></div>
         ...
         <div id="itunes-client-required" >
          <center>
            ...
            <input type="submit" value="{$API->LANG->_('Register')}"/></center>
         </div>


      <p class="footer"><a href="javascript:history.go(-1);">{$API->LANG->_('Go back')}</a> | {$API->LANG->_('If you have an account, you can')} <a href="{$API->SEO->make_link('login')}">{$API->LANG->_('login here')}</a>.</p>
