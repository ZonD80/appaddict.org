...
class Torrent {
    ...
    /* static */ public function scrape($announce = null, $hash_info = null, $timeout = self::timeout) {
        $packed_hash_nourlencoded = (pack('H*', $hash_info ? $hash_info : $this->hash_info() ));
        $packed_hash = urlencode($packed_hash_nourlencoded);
        ...
        foreach ((array) ($announce ? $announce : $this->announce()) as $tier)
            foreach ((array) $tier as $tracker) {
                ...
                $tracker = str_ireplace(array('/announce', ':80/'), array('/scrape', '/'), $tracker);
        ...
        if ($UDP_TRACKERS) {
            foreach ($UDP_TRACKERS as $url) {

                if (!preg_match('%udp://([^:/]*)(?::([0-9]*))?(?:/)?%si', $url, $m)) {
                    self::set_error(new Exception('Invalid tracker url.'),true);
                }
                $tracker = 'udp://' . $m[1];
                ...
                if (!$fp) {
                    self::set_error(new Exception('Could not open UDP connection: ' . $errno . ' - ' . $errstr), true);
                    continue;
                }
                ...
                if (strlen($ret) < 1) {
                    self::set_error(new Exception('No connection response.'), true);
                    $scrape[$url] = false;
                    continue;
                }
                if (strlen($ret) < 16) {
                    self::set_error(new Exception('Too short connection response.'), true);
                    $scrape[$url] = false;
                    continue;
                }
                $retd = unpack("Naction/Ntransid", $ret);
                if ($retd['action'] != 0 || $retd['transid'] != $transaction_id) {
                    self::set_error(new Exception('Invalid connection response.'), true);
                    $scrape[$url] = false;
                    continue;
                }
                $current_connid = substr($ret, 8, 8);

                $packet = $current_connid . pack("N", 2) . pack("N", $transaction_id) . $packed_hash_nourlencoded;
                fwrite($fp, $packet);
                ...
                if (strlen($ret) < 1) { //throw new ScraperException('No scrape response.',0,true);
                    fclose($fp); // closing file, now let's try announce! XBT provides seeders/leechers in announce!
                    ...
                    $left = $downloaded;
                    $uploaded = $downloaded;
                    $packet = $current_connid . pack("N", 1) . pack("N", $transaction_id) . $packed_hash_nourlencoded . $downloaded . $left . $uploaded . pack("N", 2) . pack("N", ip2long($_SERVER['SERVER_ADDR'])) . pack("N", 69) . pack("N", 500) . pack("N", rand(0, 65535));
                    ...
                    if (strlen($ret) < 1) {
                        self::set_error(new Exception('No announce response.'), true);
                        $scrape[$url] = false;
                        continue;
                    }
                    if (strlen($ret) < $readlength) {
                        self::set_error(new Exception('Too short announce response.'), true);
                        $scrape[$url] = false;
                        continue;
                    }
                    $retd = unpack("Naction/Ntransid", $ret);
                    ...
                    if ($retd['action'] != 2 || $retd['transid'] != $transaction_id) {
                        self::set_error(new Exception('Invalid announce response.'), true);
                        $scrape[$url] = false;
                        continue;
                    }

                    $index = 8;
                    $retd = unpack("Nincomplete/Ncomplete", substr($ret, $index, 8));
                ...
                if (strlen($ret) < 8) {
                    self::set_error(new Exception('Too short scrape response.'), true);
                    $scrape[$url] = false;
                    continue;
                }
                $retd = unpack("Naction/Ntransid", $ret);
                // Todo check for error string if response = 3
                if ($retd['action'] != 2 || $retd['transid'] != $transaction_id) {
                    self::set_error(new Exception('Invalid scrape response.'), true);
                    $scrape[$url] = false;
                    continue;
                }
                if (strlen($ret) < $readlength) {
                ...
                $retd = unpack("Ncomplete/Ndownloaded/Nincomplete", substr($ret, $index, 12));
    ...
    protected function touch($void = null) {
        $this->{'created by'} = 'AppAddict torrent operations';
